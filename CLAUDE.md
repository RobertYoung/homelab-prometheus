...
Single-host Ansible project deploying Prometheus via Docker Compose on `prometheus.local.iamrobertyoung.co.uk`. Secrets are stored in AWS SSM Parameter Store.
...
```bash
# Test connectivity
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible prometheus -m ping

# Run full playbook
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml

# Run specific role only
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml --tags prometheus
...
8. `prometheus` - Prometheus deployment (Docker Compose with systemd service)

### Prometheus Role Structure
The `roles/prometheus` role deploys Prometheus via Docker Compose:
- `tasks/main.yml` - Main task orchestration (setup, nginx, prometheus, backup)
...
- `/etc/prometheus` - Configuration files and docker-compose.yml
- `/var/lib/prometheus` - Application data
- `/etc/prometheus/certs` - TLS certificates (copied from /etc/ssl, mounted as directory into nginx)
...
### Available Tags
Run specific parts with `--tags`: `configure-system`, `shell`, `docker`, `telegraf`, `step-ca-client`, `syslog`, `wazuh-agent`, `prometheus`
